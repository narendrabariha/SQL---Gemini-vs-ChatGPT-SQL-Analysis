CREATE DATABASE GEMINI_VS_CHATGPT;

USE GEMINI_VS_CHATGPT;
...
-- Average scores for Gemini Ultra
SELECT CapabilityName, AVG(ScoreGemini) AS AvgGemini
FROM Benchmarks
JOIN Capabilities ON Benchmarks.CapabilityID = Capabilities.CapabilityID
GROUP BY CapabilityName;

-- Average scores for GPT-4
SELECT CapabilityName, AVG(ScoreGPT4) AS AvgGPT4
FROM Benchmarks
JOIN Capabilities ON Benchmarks.CapabilityID = Capabilities.CapabilityID
GROUP BY CapabilityName;
...
SELECT
    CapabilityName,
    AVG(CASE WHEN ModelID = 1 THEN ScoreGemini ELSE ScoreGPT4 END) AS AvgGemini,
    AVG(CASE WHEN ModelID = 2 THEN ScoreGemini ELSE ScoreGPT4 END) AS AvgGPT4
FROM Benchmarks
JOIN Capabilities ON Benchmarks.CapabilityID = Capabilities.CapabilityID
GROUP BY CapabilityName;
...
SELECT BenchmarkName, 
MAX(ScoreGemini) AS High_ScoreGemini, 
MAX(ScoreGPT4) AS High_ScoreGPT4
FROM Benchmarks
...
6) Benchmarks where Gemini Ultra is expected to outperform GPT-4 based on the next score:

SELECT BenchmarkName
FROM Benchmarks
WHERE ScoreGemini > COALESCE(ScoreGPT4, 0);
...
UPDATE Capabilities SET CapabilityName = UPPER(CapabilityName);
UPDATE Benchmarks SET BenchmarkName = UPPER(BenchmarkName);

SELECT BenchmarkName CapabilityName FROM Benchmarks Capabilities;

SELECT BenchmarkName, CapabilityName
FROM Benchmarks
JOIN Capabilities ON Benchmarks.CapabilityID = Capabilities.CapabilityID;
...
SELECT BenchmarkName, CONCAT(BenchmarkName, ' - ', Description) AS BenchmarkWithDescription
FROM Benchmarks;

8) Retrieve the rankings for each capability based on Gemini Ultra scores?

WITH CapabilityRankings AS 
(SELECTc.CapabilityName,
b.BenchmarkName,
b.ScoreGemini,
...
SELECT
    ...
    CASE
        ...
        WHEN ScoreGemini >= 70 THEN 'Medium'
        ELSE 'Low'
    END AS PerformanceCategoryGemini,
    CASE
        WHEN ScoreGPT4 >= 90 THEN 'High'
        WHEN ScoreGPT4 >= 70 THEN 'Medium'
        ELSE 'Low'
    END AS PerformanceCategoryGPT4
FROM Benchmarks;
...
WITH ModelAverages AS (
    SELECT
        ModelID,
        CapabilityID,
        AVG(CASE WHEN ModelID = 1 THEN ScoreGemini ELSE ScoreGPT4 END) AS AvgScore
    FROM Benchmarks
    GROUP BY ModelID, CapabilityID)
SELECT BenchmarkName
FROM Benchmarks
JOIN ModelAverages ON Benchmarks.ModelID = ModelAverages.ModelID AND Benchmarks.CapabilityID = ModelAverages.CapabilityID
WHERE ScoreGemini > AvgScore AND ScoreGPT4 > AvgScore;
...
JOIN Capabilities ON Benchmarks.CapabilityID = Capabilities.CapabilityID
WHERE ModelID = 1
GROUP BY Capabilities.CapabilityName;

SELECT
    CapabilityName,
    AVG(CASE WHEN ModelID = 1 THEN ScoreGemini ELSE ScoreGPT4 END) AS Avg_Score_Gemini,
    AVG(CASE WHEN ModelID = 2 THEN ScoreGemini ELSE ScoreGPT4 END) AS Avg_Score_GPT4
FROM Benchmarks
JOIN Capabilities ON Benchmarks.CapabilityID = Capabilities.CapabilityID
GROUP BY CapabilityName;
